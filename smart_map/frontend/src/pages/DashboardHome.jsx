...
export default function DashboardHome() {
  return (
    <div className="p-8 max-w-7xl">
      {/* Page header */}
      <div className="mb-8 pb-5 border-b border-ink-800">
        <p className="text-[11px] font-semibold tracking-widest uppercase text-accent-400 mb-1">Dashboard</p>
        <h1 className="text-2xl font-semibold text-white tracking-tight">Tổng quan hệ thống</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-ink-800 border border-ink-800">
        {modules.map((m) => {
          const Inner = (
            <div className={`h-full bg-ink-900 p-6 transition-colors ${m.enabled ? 'hover:bg-ink-850' : ''}`}>
              <div className="flex items-start justify-between mb-4">
                <span className="font-mono text-xs text-ink-400 tracking-widest">{m.code}</span>
                <span className={`text-[10px] font-semibold tracking-widest uppercase ${m.enabled ? 'text-emerald-400' : 'text-ink-400'}`}>
                  {m.status}
                </span>
              </div>
              <h3 className={`text-base font-semibold mb-2 ${m.enabled ? 'text-white' : 'text-ink-300'}`}>
                {m.title}
              </h3>
              <p className="text-sm text-ink-400 leading-relaxed">{m.desc}</p>
              {!m.enabled && (
                <div className="mt-4 pt-4 border-t border-ink-800 text-[11px] text-ink-500 uppercase tracking-widest">
                  Chưa triển khai
                </div>
              )}
            </div>
